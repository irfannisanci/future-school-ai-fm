...
export function downloadExhibitionPng(input:ExportInput){
  const initialEvaluation=evaluateDesign(input.initial);
  ...
  drawPlan(ctx,input.initial,70,190,"İlk tasarım",initialEvaluation.scores.total); drawPlan(ctx,input.final,850,190,"Son tasarım",input.evaluation.scores.total,input.evaluation.scores.total-initialEvaluation.scores.total);
